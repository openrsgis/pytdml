...
def version_converter(old_v_path):
    ...
    for data in old_v_ds["data"]:
        ...
        if data["labeling"]:
            ...
            new_labeling = [
                basic_types.AI_Labeling(
                    id=labeling["id"],
                    type="AI_Labeling",
                    labelers=(
                        [
                            basic_types.AI_Labeler(
                                id=labeler["id"],
                                name=labeler["name"],
                                type="AI_Labeler",
                            )
                            for labeler in labeling["labelers"]
                        ]
                        if labeling["labelers"]
                        else None
                    ),
                    procedure=(
                        basic_types.AI_LabelingProcedure(
                            type="AI_LabelingProcedure",
                            id=labeling["procedure"]["id"],
                            methods=labeling["procedure"]["methods"],
                            tools=labeling["procedure"]["tools"],
                        )
                        if labeling["procedure"]
                        else None
                    ),
                )
        ...
        if data["quality"]:
            ...
            new_quality = basic_types.DataQuality(
                type="DataQuality",
                scope=(
                    basic_types.MD_Scope(
                        level=quality["scope"]["level"],
                        level_description=(
                            basic_types.MD_ScopeDescription(
                                attributes=" ".join(
                                    quality["scope"]["levelDescription"]["attributes"]
                                ),
                                features=" ".join(
                                    quality["scope"]["levelDescription"]["features"]
                                ),
                                feature_instances=" ".join(
                                    quality["scope"]["levelDescription"][
                                        "featureInstances"
                                    ]
                                ),
                                attribute_instances=" ".join(
                                    quality["scope"]["levelDescription"][
                                        "attributeInstances"
                                    ]
                                ),
                            ...
                            if quality["scope"]["levelDescription"]
                            else None
                        ),
                    )
                    if quality["scope"]
        ...
        if data["dateTime"] and isinstance(data["dateTime"], str):
            ...
            data_time = None
        new_data.append(
            extended_types.AI_EOTrainingData(
    ...
    if old_v_ds["dataSources"]:
        ...
        new_data_source = [
            basic_types.CI_Citation(title=data_source) for data_source in data_sources
        ]
    else:
    ...
    if old_v_ds["metricsInLIT"]:
        ...
        new_metrics_in_LIT = [
            basic_types.AI_MetricsInLiterature(
                doi=item["doi"],
                metrics=(
                    basic_types.NamedValue(
                        key=item["metrics"]["key"], value=item["metrics"]["value"]
                    )
                    if item["metrics"]
                    else None
                ),
    ...
    if old_v_ds["classes"]:
        ...
        if isinstance(classes[0], dict):
            ...
            new_classes = [
                basic_types.NamedValue(key=classes[i], value=i + 1)
                for i in range(len(classes))
            ]
    else:
        new_classes = None
    ...
    if old_v_ds["statisticsInfo"]:
        ...
        new_info = [
            basic_types.NamedValue(
                key=next(iter(info.items()))[0], value=next(iter(info.items()))[1]
            )
            ...
            level_description=(
                [
                    basic_types.MD_ScopeDescription(
                        ...
                        feature_instances=(
                            " ".join(levelDescription["featureInstances"])
                            if levelDescription["featureInstances"]
                            else None
                        ),
                        attribute_instances=(
                            " ".join(levelDescription["attributeInstances"])
                            if levelDescription["attributeInstances"]
                            else None
                        ),
                        dataset=levelDescription["dataset"],
                        other=levelDescription["other"],
                    )
                    for levelDescription in scope["levelDescription"]
                ]
                if scope["levelDescription"]
                else None
            ),
        )
    else:
        ...
        new_labeling = [
            basic_types.AI_Labeling(
                ...
                labelers=(
                    [
                        basic_types.AI_Labeler(
                            id=labeler["id"], name=labeler["name"], type="AI_Labeler"
                        )
                        for labeler in labeling["labelers"]
                    ]
                    if labeling["labelers"]
                ...
                basic_types.MD_Scope(
                    ...
                    level_description=(
                        basic_types.MD_ScopeDescription(
                            ...
                            feature_instances=" ".join(
                                quality["scope"]["levelDescription"]["featureInstances"]
                            ),
                            attribute_instances=" ".join(
                                quality["scope"]["levelDescription"][
                                    "attributeInstances"
    ...
    if old_v_ds["bands"]:
        ...
        new_bands = [
            basic_types.MD_Band(name=[basic_types.MD_Identifier(code=band)])
            for band in bands
        ]
    else:
        new_bands = None
    ...
    new_v_ds = extended_types.EOTrainingDataset(
        ...
        providers=(
            [old_v_ds["providers"]]
            if isinstance(old_v_ds["providers"], str)
            else old_v_ds["providers"]
        ),
        scope=new_scope,
