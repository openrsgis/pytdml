...
class ResNet_BasicBlock_OS16(nn.Module):
    def __init__(self, num_layers):
        ...
        if num_layers == 18:
            ...
            resnet.load_state_dict(
                torch.load(
                    "/home/pyy/pytdml-main/examples/semantic_segmentation/DeepLabV32/pretrainNet/resnet18-5c106cde.pth"
                )
            )
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            resnet.load_state_dict(
                torch.load(
                    "/home/pyy/pytdml-main/examples/semantic_segmentation/DeepLabV32/pretrainNet/resnet34-333f7ec4.pth"
                )
            )
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
